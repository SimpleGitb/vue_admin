   ...
   fetchData(){
    ...
    rowdbClick(row, column, cell, event){
      if(column.label=='文件名'){
        ...
        this.$axios.get(process.env.API_HOST+"/api/AppInfoList/show?id="+row._id.$oid,this.config).then((res) => {
    ...
    filterChange(filters){
      ...
      if(this.input5){
        this.$axios.get(process.env.API_HOST+"/api/AppInfoList/search?list_source="+this.listValue+"&condition="+this.filename+"&keyword="+this.input5+"&filter_condition[label_name]="+this.filter1+"&filter_condition[file_type]="+this.filter2+"&filter_condition[list_source]="+this.filter3+"&filter_condition[system_source]="+this.filter4+"&filter_condition[list_type]="+this.filter5+"&page="+this.currentPage+"&pageSize="+this.pageSize,this.config).then((res) => {
            this.total = 0;
            this.tableData = [];
            if(res.data.data){
              this.tableData=res.data.data;
    ...
    listValueFilter(){
      this.$axios.get(process.env.API_HOST+"/api/AppInfoList/search?list_source="+this.listValue+"&page="+this.currentPage+"&pageSize="+this.pageSize,this.config).then((res) => {
            this.total = 0;
            this.tableData = [];
            if(res.data.data){
              this.tableData=res.data.data;
