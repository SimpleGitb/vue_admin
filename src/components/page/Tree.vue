    ...
    <div class="all-div" v-if="model.name">
      <div class="itemRow" :style="{ marginLeft:model.level*20+'px' }">
        <span v-show="model.children.length" @click="expandOrCollapse">
          <i v-if="model.isOpen" class="iconfont icon-shanchu"></i>
          <i v-else class="iconfont icon-bi"></i>
        </span>
        <div class="hover-div" @mouseover="flag=true" @mouseout="flag=false">
          <span @click="jump(model.url)">{{model.name}}</span>
          <span v-show="flag==true" @click="add"><i class="iconfont icon-bi"></i></span>
          <span v-show="flag==true" @click="remove(model)"><i class="iconfont icon-bi"></i></span>
          <span v-show="flag==true" @click="edit"><i class="iconfont icon-bi"></i></span>
